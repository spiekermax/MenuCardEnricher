package de.unihannover.hci.menudetector.views
...
class GraphicOverlayView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
     ...
     */
    abstract class Graphic(private val overlay: GraphicOverlayView) {
        ...
        fun updatePaintColorByZValue(
            ...
            if (zInScreenPixel < 0) {
                ...
                val v = (zInScreenPixel / zLowerBoundInScreenPixel * 255).toInt().coerceIn(0..255)
                paint.setARGB(255, 255, 255 - v, 255 - v)
            } else {
                // Sets up the paint to be blue if the item is behind the z origin.
                // Maps values within [0, zUpperBoundInScreenPixel] to [0, 255] and use it to control the
                // color. The larger the value is, the more blue it will be.
                val v = (zInScreenPixel / zUpperBoundInScreenPixel * 255).toInt().coerceIn(0..255)
    ...
    fun setImageSourceInfo(imageWidth: Int, imageHeight: Int, isFlipped: Boolean) {
        if (imageWidth <= 0)
            throw IllegalArgumentException("Image width must be positive")
        if (imageHeight <= 0)
            throw IllegalArgumentException("Image height must be positive")
